...
async def on_message(message):
    ...
    if message.author == client.user:
        return
    if message.content.startswith('price'):#獲取報價
        ...
        if len(message.content.split(" ")) == 2:
         ...
         while i < len(require):
           ...
           i = i + 1
    if message.content.startswith('fprice'):#獲取合約報價
        ...
        if len(message.content.split(" ")) == 2:
         ...
         while i < len(require):
           ...
           await message.channel.send(require[i] + ":" + response.text[pos:pos+8])
           i = i + 1
    if message.content.startswith('binprice'):#獲取更完整爆價
        if len(message.content.split(" ")) == 2:
    ...
    if message.content.startswith('bindeposit'):#顯示目前帳號的地址 需要提供幣種
        if len(message.content.split(" ")) == 2:
    ...
    if message.content.startswith('MCserver'):#輸出林品豪架設的MC server的資訊
        await message.channel.send('https://web.nttu.edu.tw/s31/MinecraftServer/server.php')

    if message.content.startswith('MCbuild'):#一些影片
        await message.channel.send('https://youtu.be/uldhfEZydTo')
    
    if message.content.startswith('TTweather'):#台東天氣
    ...
    if message.content.startswith('date'):#現在時間
    ...
    if message.content.startswith('python Docs'):#顯示python docs
    ...
    if message.content.startswith('cpp Docs'):#顯示cpp docs
    ...
    if message.content.startswith('UVI'):#顯示台灣各地的UV
    ...
    if message.content.startswith('binfuex') and message.content.split()[1]!="test":#下單的部分
        try:
    ...
    if message.content.startswith('binasset'):#查看剩下多少錢
    ...
    if message.content.startswith('binleverange'):#調整槓桿
    ...
    if message.content.startswith('testbinorder'):#測試下單用的
        try:
            qu=float(message.content.split()[3])
